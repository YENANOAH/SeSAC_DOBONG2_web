// 문자열에서 사용하는 속성과 메소드 
// length 
// toUpperCase, toLowerCase, trim, indesOF, slice
...
console.log(fruit.charAt(10)) //아무것도 안나옴 '' 빈문자열
...
let date = "2024.11.06" // yyyy-mm-dd
...
console.log.apply(arr1.pop()) // 제거하는 값 반환
arr1.pop()
arr1.pop()
console.log(arr1)
...
// 배열.includes(요소) 배열에 요소가 이쓴ㄴ지 없는지 확인
...
// join(''), 배열 > 문자열로 병합
console.log(str1) // join의 인자에 아무것도 전달이 되지 않으면 배열 안의 콤마까지 같이 문자열로 반환됨
str1 = arr1.join()
str1 = arr1.join("")
console.log(str1)
...
console.log('------filter 조건에 맞는 모든 값--------')
...
for(let area in areaNUm){
    console.log(area) // key > 문자열로 반환중 값에 접근? 대괄호 접근법으로만 사용가능
